...
$script:SarmaConfig = @{
    ...
    LocalRepo       = if ($env:SARMA_LOCAL_REPO) { $env:SARMA_LOCAL_REPO } else { "" }
    ...
    CopilotCliCmd   = if ($env:COPILOT_CLI_CMD) { $env:COPILOT_CLI_CMD } else { "copilot-cli" }
    WorkerTaskTypes = if ($env:WORKER_TASK_TYPES) { $env:WORKER_TASK_TYPES -split "," } else { @("backend","frontend","test","docs") }
